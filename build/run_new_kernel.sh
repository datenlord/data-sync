...
if [ $BUILD_BCC = "true" ]; then
    # Install BCC build dependencies
    #    git \
    #    zlib1g-dev \
    sudo -E apt install --yes \
        bison \
        build-essential \
        cmake \
        flex \
    ...
    # Build BCC
    cd $WORK_DIR
    [ ! -d $WORK_DIR/bcc ] && git clone --single-branch https://github.com/iovisor/bcc.git
...
docker build $DOCKER_BUILD_DIR \
    ...
    --file $WORK_DIR/build/Dockerfile.test_env \
...
docker run \
    ...
    -it \
    -v $WORK_DIR:$WORK_DIR \
    -e ADD_CARGO_REPO_MIRROR=$ADD_CARGO_REPO_MIRROR \
    -e BCC=$BCC \
    -e FEATURES=$FEATURES \
    -e LLVM=$LLVM \
    -e WORK_DIR=$WORK_DIR \
    ...
    /bin/bash -e $WORK_DIR/build/test_in_kata.sh \
    ;
